...
public class VehicleEntryCommandHandlerTests
{
  private readonly Mock<IEstablishmentRepository> _establishmentRepositoryMock;
  private readonly Mock<IVehicleRepository> _vehicleRepositoryMock;
  private readonly VehicleEntryCommandHandler _handler;

  public VehicleEntryCommandHandlerTests()
  {
    _establishmentRepositoryMock = new Mock<IEstablishmentRepository>();
    _vehicleRepositoryMock = new Mock<IVehicleRepository>();
    _handler = new VehicleEntryCommandHandler(_establishmentRepositoryMock.Object, _vehicleRepositoryMock.Object);
  }
  ...
  {
    // Arrange
    var command = new VehicleEntryCommand(Guid.NewGuid(), Guid.NewGuid());
  ...
  public async Task Handle_ReturnsFailure_WhenVehicleNotFound()
  {
    ...
    var command = new VehicleEntryCommand(establishmentId, vehicleId);
  ...
  [Fact]
  public async Task Handle_ReturnsSuccess_WhenEntrySucceeds()
  {
    // Arrange
    var establishmentId = Guid.NewGuid();
    var vehicleId = Guid.NewGuid();
    ...
    _establishmentRepositoryMock.Verify(r => r.GetByIdAsync(establishmentId, It.IsAny<CancellationToken>()), Times.Once);
    _vehicleRepositoryMock.Verify(r => r.GetByIdAsync(vehicleId, It.IsAny<CancellationToken>()), Times.Once);
    _establishmentRepositoryMock.Verify(r => r.UpdateAsync(establishment, It.IsAny<CancellationToken>()), Times.Once);
  }
}
